    ...
    for (let s of this.slices) {
      if (s.state === 'entry') {
        ...
        if (s.entryDelay > 0) s.entryDelay--;
          ...
          s.offsetX += (s.targetOffsetX - s.offsetX) * 0.09;
          s.offsetY += (s.targetOffsetY - s.offsetY) * 0.09;
        }
        if (dist(mouseX, mouseY, this.x, this.y) < 100) s.state = 'revealing';
      } else if (s.state === 'revealing') {
        s.offsetX += (0 - s.offsetX) * 0.12;
        s.offsetY += (0 - s.offsetY) * 0.12;
        ...
        if (s.delay > 0) s.delay--;
          ...
          s.offsetX += s.dirX * s.exitSpeed;
          s.offsetY += s.dirY * s.exitSpeed;
        }
      }
    }
  }
